    ...
    public void enrich(Object testCase) {
        for (Field field : SecurityActions.getFieldsWithAnnotation(testCase.getClass(), ArquillianResource.class)) {
            ...
            try {
                ...
                throw new RunCustomException("Could not set value on field " + field + " using " + value, e);
            }
        }
    }
    ...
    private Object lookup(Class<?> type, ArquillianResource resource, List<Annotation> qualifiers) {
        ...
        for (ResourceProvider resourceProvider : resourceProviders) {
            if (resourceProvider.canProvide(type)) {
                ...
                if (value == null) {
                    if (type == null || resourceProvider == null) {
                        logger.warning("Provider for type or resourceProvider is null");
                    } else {
                        logger.warning(String.format("Provider for type %s returned a null value: %s", type, resourceProvider));
                    }                    
