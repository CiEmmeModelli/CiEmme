...
public class JUnitIntegrationWithRuleTestCase extends JUnitTestBaseClass {
    ...
    public void shouldWorkWithExpectedException() throws Exception {
        ...
        Result result = run(adaptor, TestClassWithArquillianClassAndMethodRuleWithExpectedException.class);
    ...
    public void shouldThrowMultipleExceptionsWhenBeforeAndAfterThrowException() throws Exception {
        ...
        Result result = run(adaptor, TestClassWithArquillianClassAndMethodRuleWithExceptionInBeforeAndAfter.class);

        Assert.assertFalse(result.wasSuccessful());
        Assert.assertEquals(2, result.getFailureCount());
        Assert.assertEquals("BeforeException", result.getFailures().get(0).getMessage());
        Assert.assertEquals("AfterException", result.getFailures().get(1).getMessage());
        assertCycle(1, Cycle.BEFORE_CLASS, Cycle.BEFORE, Cycle.AFTER, Cycle.AFTER_CLASS);
        assertCycle(0, Cycle.TEST);
    ...
    public void shouldCallAfterRuleIfFailureInBeforeRule() throws Exception {
        ...
        Result result = run(adaptor, ClassWithArquillianClassAndMethodRuleWithExceptionInBeforeRuleTestCase.class);
    ...
    public void shouldThrowMultipleExceptionIfFailureInBeforeAndAfterRule() throws Exception {
        ...
        Assert.assertEquals("AfterRuleException",result.getFailures().get(1).getMessage());
        assertCycle(1, Cycle.basics());
